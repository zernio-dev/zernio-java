...
public class Example {
    public static void main(String[] args) {
        ...
        LocalDate since = LocalDate.now(); // LocalDate | Meta only. Earliest delivery date (YYYY-MM-DD). LinkedIn's archive does not filter by date, so it is a 400 there: filter on details.adStatistics.firstImpressionAt / latestImpressionAt instead (EU-delivered ads only).
        LocalDate until = LocalDate.now(); // LocalDate | Meta only. Latest delivery date (YYYY-MM-DD); a 400 on LinkedIn, see since.
...
| **since** | **LocalDate**| Meta only. Earliest delivery date (YYYY-MM-DD). LinkedIn&#39;s archive does not filter by date, so it is a 400 there: filter on details.adStatistics.firstImpressionAt / latestImpressionAt instead (EU-delivered ads only). | [optional] |
| **until** | **LocalDate**| Meta only. Latest delivery date (YYYY-MM-DD); a 400 on LinkedIn, see since. | [optional] |
