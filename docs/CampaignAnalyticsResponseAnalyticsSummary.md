...
|**allConversions** | **BigDecimal** | All conversions, including actions excluded from the Conversions column (Google metrics.all_conversions). 0 on platforms without the concept. |  [optional] |
...
|**searchImpressionShare** | **BigDecimal** | Google only. Date-range ratio, not a percentage. Null when unavailable; Google&#39;s threshold sentinel values are preserved. |  [optional] |
|**searchBudgetLostImpressionShare** | **BigDecimal** | Google only. Date-range ratio, not a percentage. Null when unavailable; Google&#39;s threshold sentinel values are preserved. |  [optional] |
|**searchRankLostImpressionShare** | **BigDecimal** | Google only. Date-range ratio, not a percentage. Null when unavailable; Google&#39;s threshold sentinel values are preserved. |  [optional] |
|**searchTopImpressionShare** | **BigDecimal** | Google only. Date-range ratio, not a percentage. Null when unavailable; Google&#39;s threshold sentinel values are preserved. |  [optional] |
|**searchAbsoluteTopImpressionShare** | **BigDecimal** | Google only. Date-range ratio, not a percentage. Null when unavailable; Google&#39;s threshold sentinel values are preserved. |  [optional] |
